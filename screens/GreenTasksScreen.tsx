...
type GreenTasksScreenNavigationProp = NativeStackNavigationProp<
  RootStackParamList,
  'GreenTasksScreen'
>;

export default function GreenTasksScreen() {
  const navigation = useNavigation<GreenTasksScreenNavigationProp>();
  ...
  const [selectedUrgency, setSelectedUrgency] = useState<string>('green'); // default green
  ...
  async function fetchTasks() {
    ...
    setUserId(uid);
    // Fetch tasks with urgency 'green' and pending status.
  ...
  function handleEditTask(task: Task) {
    ...
    setSelectedUrgency(task.urgency || 'green');
  ...
  function renderTaskItem({ item }: { item: Task }) {
    ...
    return (
      <View style={[styles.taskCard, { backgroundColor: '#e6ffe6' }]}>
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.header}>Green Tasks</Text>
      {isLoading ? (
        <Text style={styles.loadingText}>Loading tasks...</Text>
      ) : tasks.length === 0 ? (
        <Text style={styles.noTasks}>No green tasks yet.</Text>
      ) : (
        <FlatList
          data={tasks}
...
const styles = StyleSheet.create({
  ...
  taskCard: {
    flexDirection: 'row',
    backgroundColor: '#e6ffe6', // Light green background for cards
    borderRadius: 10,
    padding: 15,
    marginVertical: 8,
    alignItems: 'center',
